...
export class FormScene {
  ...
  async leaveScene(@Ctx() ctx, @Form() form) {
    ...
    await ctx.reply(
      'Вы успешно завершили заполнение анкеты. Теперь вы можете пользоваться остальными функциями бота. Введите /help, чтобы узнать все возможности.',
    );
